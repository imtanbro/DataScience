...
def get_filters():
    ...
    while (1):
        if (city=='chicago' or city=='new york city' or city=='washington'):
            ...
            print('\nInvalid input!')
            city=input('\nPlease enter a valid city name\n Valid cities are Chicago, New York City or Washington: ').lower()

    # TO DO: get user input for month (all, january, february, ... , june)
    month_list=['all','january','february','march','april','may','june']
    month=input('\nEnter the month name (All,January, February, March, April, May, June): ').lower()
    while(1):
        if(month in month_list):
            break
        else:
            print('Invalid input!')
            month=input('\nPlease enter a valid month name from (All,January, February, March, April, May, June): ').lower()
            
    # TO DO: get user input for day of week (all, monday, tuesday, ... sunday)
    days_list=['all','sunday','monday','tuesday','wednesday','thursday','friday','saturday']
    day=input('\nEnter the day of the week(All, Monday, Tuesday, ... Sunday): ').lower()
    while(1):
        if(day in days_list):
            break
        else:
            print('\nInvalid input!')
            day=input('\nPlease enter a valid day from(All, Monday, Tuesday, ... Sunday): ').lower()
    ...
    if choice:
        while 1:
            for i in range(5):
                print(df.iloc[i])
                print("\n")
            choice = input('Want to see more? Yes/No ').lower()
            if choice=='yes' or choice=='y':
                continue
            elif choice=='no' or choice=='n':
                break
            else:
                print('\nOops! I dont understand that \n Output expected is Yes/No.')
                return
...
def main():
    while True:
        ...
        user_stats(df)
        display_data(df)
        restart = input('\nWould you like to go for one more round? Enter yes or no.\n')
